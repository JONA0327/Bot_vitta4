...
@app.post("/webhook")
async def webhook(request: Request, background_tasks: BackgroundTasks, x_bot_secret: str | None = Header(default=None)) -> JSONResponse:
    ...
    background_tasks.add_task(_procesar_en_segundo_plano, payload)
    return JSONResponse({"status": "recibido"})


async def _procesar_en_segundo_plano(payload: dict) -> None:
    try:
        await procesar_mensaje(payload, prompts)
    except Exception:
        logger.exception("Error procesando mensaje: %s", payload.get("telefono"))
